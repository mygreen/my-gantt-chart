      ...
      {layouts.slice(visibleWindow.start, visibleWindow.end).map((layout, offsetIndex) => {
        ...
        const widthDelta =
          resizeState?.taskId === task.id && resizeState.edge === "start"
            ? -startResizeOffset
            : endResizeOffset;
        const previewWidth = Math.max(dayWidth, layout.width + widthDelta);
        const previewTranslateX =
          layout.x + dragOffset + (resizeState?.taskId === task.id && resizeState.edge === "start"
        ...
        return (
          <div
            key={task.id}
            className={cn(
              "absolute overflow-hidden rounded-md border bg-white shadow-sm transition-shadow",
          ...
          >
                  ...
                  {progressSegments.length > 0 ? (
                    progressSegments.map((segment, index) => (
                      <div
                        ...
                        className={cn(
                          "absolute inset-y-0",
                          !segment.isWorkingDay && excludeNonWorkingDays && "bg-slate-300/70",
                        )}
                        style={{
            ...
            <button
              ...
              onPointerMove={(event) => {
                ...
                const dayOffset = Math.round((event.clientX - resizeState.startClientX) / dayWidth);
